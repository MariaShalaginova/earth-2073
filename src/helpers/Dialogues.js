const dialogues = [
    {id: 0,
      characterName: 'Алиса',
      replies: [
        {
          text: 'Привет, ребята!',
          background: 'classroom.jpg',
    
        },
        {
          text: 'Это удивительно!',
          background: 'фон2.jpg'
        },
        // Другие реплики персонажа 
      ],
      scene: 'Сцена 1',
      characterImages: 'kids.jpg'
    },
    {
      characterName: 'Денис',
      replies: [
        {
          text: 'Да, супер! Концовка- отпад. ',
          background: 'classroom.jpg'
        },
        {
          text: 'А я как-то слышал, как он с кем-то по телефону говорил о каких-то роботах в теплицах. Интересно, что он хочет нам сейчас тут рассказать...',
          background: 'classroom.jpg'
        },
        // Другие реплики персонажа 
      ],
      scene: 'Сцена 1',
      characterImages: 'kids.jpg'
    },
    {
      characterName: 'Артем',
      replies: [
        {
          text: 'Денис, как твоя новая игра? Понравилась? ',
          background: 'classroom.jpg'
        },
        {
          text: 'Странный этот новый учитель биологии Максим Николаевич. Очень странный!',
          background: 'classroom.jpg'
        },
        // Другие реплики персонажа 
      ],
      scene: 'Сцена 1',
      characterImages: 'kids.jpg'
    },
    {
        characterName: 'Элина',
        replies: [
          {
            text: 'О, привет, Алиса! ',
            background: 'classroom.jpg'
          },

          // Другие реплики персонажа 
        ],
        scene: 'Сцена 1',
        characterImages: 'kids.jpg'
    },
    {
        characterName: 'Марина',
        replies: [
          {
            text: 'Ну? и как вы думаете, зачем нас попросили остаться после уроков? ',
            background: 'classroom.jpg'
          },
           // Другие реплики персонажа 
        ],
        scene: 'Сцена 1',
        characterImages: 'kids.jpg'
    },
    // {
    //     characterName: 'Максим',
    //     replies: [
    //       {
    //         text: '',
    //         background: 'classroom.jpg'
    //       },
    //     ],
    //     scene: 'Сцена 1',
    //     characterImages: 'Max.jpg'
    //   },
  ];
  
  export default dialogues;

  const dialogues1 = [
    {
      characterName: 'Алиса',
      replies: [
        {
          text: 'Привет, ребята!',
          background: 'classroom.jpg',
          scene: 'Сцена 1', 
          characterImages: 'kids.jpg'
        },
        {
          text: 'Это удивительно!',
          background: 'classroom.jpg',
          scene: 'Сцена 1', 
          characterImages: 'kids.jpg'
        },
        {
            text: 'Выглядит, конечно, странно ',
            background: 'classroom.jpg',
            scene: 'Сцена 2', 
            characterImages: 'Alisa-2.jpg'
        },
        {
            text: 'Здравствуйте! Вам что-то подсказать?',
            background: 'classroom.jpg',
            scene: 'Сцена 2', 
            characterImages: 'Alisa-2.jpg'
        },
        {
            text: 'Похоже на фантастику. Но если это правда, мы открыты новым идеям и возможностям. ',
            background: 'classroom.jpg',
            scene: 'Сцена 2', 
            characterImages: 'Alisa-2.jpg'
        },
      ]
    },
    {
      characterName: 'Денис',
      replies: [
        {
          text: 'Да, супер! Концовка- отпад. ',
          background: 'classroom.jpg'
        },
        {
          text: 'А я как-то слышал, как он с кем-то по телефону говорил о каких-то роботах в теплицах. Интересно, что он хочет нам сейчас тут рассказать...',
          background: 'classroom.jpg'
        },
        // Другие реплики персонажа 
      ],
      scene: 'Сцена 1',
      characterImages: 'kids.jpg'
    },
    {
      characterName: 'Артем',
      replies: [
        {
          text: 'Денис, как твоя новая игра? Понравилась? ',
          background: 'classroom.jpg'
        },
        {
          text: 'Странный этот новый учитель биологии Максим Николаевич. Очень странный!',
          background: 'classroom.jpg'
        },
        // Другие реплики персонажа 
      ],
      scene: 'Сцена 1',
      characterImages: 'kids.jpg'
    },
    {
        characterName: 'Элина',
        replies: [
          {
            text: 'О, привет, Алиса! ',
            background: 'classroom.jpg'
          },

          // Другие реплики персонажа 
        ],
        scene: 'Сцена 1',
        characterImages: 'kids.jpg'
    },
    {
        characterName: 'Марина',
        replies: [
          {
            text: 'Ну? и как вы думаете, зачем нас попросили остаться после уроков? ',
            background: 'classroom.jpg'
          },
           // Другие реплики персонажа 
        ],
        scene: 'Сцена 1',
        characterImages: 'kids.jpg'
    },
    // {
    //     characterName: 'Максим',
    //     replies: [
    //       {
    //         text: '',
    //         background: 'classroom.jpg'
    //       },
    //     ],
    //     scene: 'Сцена 1',
    //     characterImages: 'Max.jpg'
    //   },
  ];
  


  